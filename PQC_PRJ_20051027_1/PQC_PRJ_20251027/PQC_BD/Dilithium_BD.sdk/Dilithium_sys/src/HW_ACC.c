...
void NTT_Hardware(int32_t a[N])
{
	...
	Xil_DCacheInvalidateRange((UINTPTR)a, 256*4);
}
...
void poly_pointwise_Hardwarey(poly *c, const poly *a, const poly *b)
{
	...
	Xil_DCacheInvalidateRange((UINTPTR)c, 256*4);
}
...
void polyvec_matrix_PWM_Hardware(polyveck mat_temp[L], const polyvecl mat[K], const polyvecl *v)
{
	...
	for(int k = 0; k < L; ++k){
		...
		Xil_DCacheInvalidateRange((UINTPTR)&mat_temp[k].vec[0], K*256*4);
	}
}
...
void polyvec_matrix_PWA_Hardware(polyveck *t, const polyveck mat_temp[L])
{
	int status;
	for(int k = 0; k < K; ++k){
		My_DMA_Ctrl_0 = 3;		// Start PWA
		My_DMA_Ctrl_6 = 0;		// Dilithium; PQC mode
		My_DMA_Ctrl_1 = ( (0<<5) + ((L-1)<<6) );	// PWA mode + PWA vector size = L
		...
		Xil_DCacheInvalidateRange((UINTPTR)&t->vec[k], 256*4);
	}
}
...
void polyvecl_pointwise_Hardware(polyvecl *r, const poly *a, const polyvecl *v)
{
	...
	Xil_DCacheInvalidateRange((UINTPTR)&r->vec[0], 256*L*4);
}
...
void polyveck_pointwise_Hardware(polyveck *r, const poly *a, const polyveck *v)
{
	...
	Xil_DCacheInvalidateRange((UINTPTR)&r->vec[0], 256*K*4);
}

void poly_PWA_Hardware(poly *c, const poly *a, const poly *b)
{
	int status;
	My_DMA_Ctrl_0 = 3;		// Start PWA/PWS
	My_DMA_Ctrl_6 = 0;		// Dilithium; PQC mode
	My_DMA_Ctrl_1 = ( (0<<5) + (1<<6) );	// PWA mode + PWA vector size
...
{
	for(int i = 0; i < K; ++i)
		poly_PWA_Hardware(&w->vec[i], &u->vec[i], &v->vec[i]);
}
...
void poly_PWS_Hardware(poly *c, const poly *a, const poly *b)
{
	...
	My_DMA_Ctrl_0 = 3;		// Start PWA/PWS
	My_DMA_Ctrl_6 = 0;		// Dilithium; PQC mode
	My_DMA_Ctrl_1 = ( (1<<5) + (1<<6) );	// PWS mode + PWS vector size
...
{
	for(int i = 0; i < K; ++i)
		poly_PWS_Hardware(&w->vec[i], &u->vec[i], &v->vec[i]);
}

void shake256_Hardware(uint8_t *out, size_t outbytes, const uint8_t *in, size_t inbytes)
{
	int status;
	My_DMA_Ctrl_6 = 3;			//SHAKE256 mode
	My_DMA_Ctrl_3 = inbytes;	//SHA3 input length
	My_DMA_Ctrl_4 = outbytes;	//SHA3 output length
	My_DMA_Ctrl_0 = 4;			//Start SHA3
	...
	Xil_DCacheInvalidateRange((UINTPTR)out, outbytes);
}
...
void shake256_Dual_Hardware(uint8_t *out, size_t outbytes, const uint8_t *in1, size_t inbytes1, const uint8_t *in2, size_t inbytes2)
{
	int status;
	My_DMA_Ctrl_6 = 3;			//SHAKE256 mode
	My_DMA_Ctrl_3 = inbytes1 + inbytes2;	//SHA3 input length
	My_DMA_Ctrl_4 = outbytes;	//SHA3 output length
	My_DMA_Ctrl_0 = 4;			//Start SHA3
...
{
	int status;
	My_DMA_Ctrl_6 = 18;			//SHAKE128 + Dilithium_rej_samp
	if (ETA == 4)
		My_DMA_Ctrl_7 = 1;		//Samp_eta
	else
		My_DMA_Ctrl_7 = 0;		//Samp_eta
	My_DMA_Ctrl_3 = 34;			//SHA3 input length
	My_DMA_Ctrl_0 = 4;			//Start SHA3
...
{
	int status;
	My_DMA_Ctrl_6 = 10;		//SHAKE128 + Dilithium_uni_samp
	My_DMA_Ctrl_3 = 34;		//SHA3 input length
	My_DMA_Ctrl_4 = 0;		//SHA3 output length
	My_DMA_Ctrl_0 = 4;		//Start SHA3
...
void polyvec_matrix_PWM_Hardware_new(polyveck mat_temp[L], const polyvecl *v)
{
	...
	for(int k = 0; k < L; ++k)
	{
		My_DMA_Ctrl_0 = 2;		// Start PWM
		My_DMA_Ctrl_6 = 0;		// Dilithium; PQC mode
		My_DMA_Ctrl_1 = K<<1;	// K = 4/6/8; pwm vector size
...
void polyvec_matrix_PWM_Hardware_sign(polyveck mat_result[L], polyveck mat[L], const polyvecl *v)
{
	...
	for(int k = 0; k < L; ++k)
	{
		...
		Xil_DCacheInvalidateRange((UINTPTR)&mat_result[k].vec[0], K*256*4);
	}
}
...
void print_polyveck(const polyveck *v, const char *name) {
	...
	for(int i = 0; i < K; i++) {
		printf("%s.vec[%d]:\n", name, i);
	    for(int j = 0; j < N; j++) {
	    	printf("%6d ", v->vec[i].coeffs[j]);
	    	if((j+1) % 16 == 0) printf("\n");
	    }
	    printf("\n");
	}
	printf("=== End of %s Array ===\n", name);
}
