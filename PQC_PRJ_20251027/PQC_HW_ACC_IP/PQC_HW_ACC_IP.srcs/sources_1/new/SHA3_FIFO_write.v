...
module SHA3_FIFO_write(
    ...
    wire	[10:0]		Dilithum_upper_shake256_gamma;
	...
	wire				Dilithum_shake256_gamma_eff_wd;
    wire				Dilithum_shake256_gamma_eff_rd;
    
	reg				    Dilithum_shake256_gamma_eff_rd_reg0;
	reg				    Dilithum_shake256_gamma_eff_rd_reg1;
	reg				    Dilithum_shake256_gamma_eff_rd_reg2;
	                                                
	reg				    Dilithum_shake256_gamma_eff_wd_reg0;
	reg				    Dilithum_shake256_gamma_eff_wd_reg1;
	reg				    Dilithum_shake256_gamma_eff_wd_reg2;
	reg				    Dilithum_shake256_gamma_eff_wd_reg3;
	reg				    Dilithum_shake256_gamma_eff_wd_reg4;	
	...
	wire	[23:0]		Kyber_out_shake128_Uni;	//从shake128输出的1344位的out_keep里每次提取的24位数据块
	wire	[11:0]		Kyber_out_shake256_CBD;	//kyber里的rej要用shake256
	wire	[47:0]		Dilithum_out_shake128_Uni;	//从shake128输出的1344位的out_keep里每次提取的48位数据块
	wire	[7:0]		Dilithum_out_shake128_Rej;
	wire	[71:0]		Dilithum_out_shake256_gamma;
	//wire	[35:0]		Dilithum_out_shake256_gamma;
	wire	[71:0]		Dilithum_out_sample_gamma;
	wire                rd_en;
    ...
    wire	[2:0]		Rej_mod5_n2;
 	...
 	reg		[31:0]		out_Rej_2;
 	reg		[17:0]		out_gamma_1;
 	reg		[17:0]		out_gamma_2;
 	reg		[17:0]		out_gamma_3;
 	reg		[17:0]		out_gamma_4;
 	reg		[35:0]      out_gamma;
    wire	[35:0]		out_gamma_final; 
    ...
    assign Kyber_out_shake256_CBD = (Samp_eta == 1'b0) ? out_keep[Kyber_upper_shake256_CBD -: 8] : ((count_shake[0] == 0) ? {out_keep[Kyber_upper_shake256_CBD - 12 -: 4], out_keep[Kyber_upper_shake256_CBD -: 8]} : {out_keep[Kyber_upper_shake256_CBD - 4 -: 8], out_keep[Kyber_upper_shake256_CBD + 4 -: 4]});//	assign Kyber_out_shake256_CBD_reorder = (Samp_eta == 1'b0) ? {Kyber_out_shake256_CBD[7:0],Kyber_out_shake256_CBD[15:8],Kyber_out_shake256_CBD[23:16]};
    ...
    // Dilithum shake256  gen mask
    assign Dilithum_upper_shake256_gamma =  11'd1343 - (count_shake * 72) ;
    assign Dilithum_out_shake256_gamma =  out_keep[Dilithum_upper_shake256_gamma -: 72]  ;
//    assign Dilithum_shake256_gamma_eff =  (count_shake < 8'd15)  && (nex_state == 2'd2); 
//	assign Dilithum_shake256_gamma_eff_rd =  (count_shake < 8'd15)  && (nex_state == 2'd2); 
    assign Dilithum_shake256_gamma_eff_wd =  (count_shake < 8'd15)  && (nex_state == 2'd2); 
	//assign Dilithum_upper_shake256_gamma = (Samp_gamma == 1'b0) ? 11'd1343 - (count_shake * 36) : 11'd1343 - (count_shake * 40);
	//assign Dilithum_out_shake256_gamma = (Samp_gamma == 1'b0) ? out_keep[Kyber_upper_shake256_CBD -: 36] : out_keep[Kyber_upper_shake256_CBD -: 40] ;
	//assign Dilithum_shake256_gamma_eff = ((Samp_gamma == 1'b0) ? (count_shake < 8'd36) : (count_shake < 8'd40)) && (nex_state == 2'd2); 

	//assign is_rej_samp = SHA3_mode_buf == 4'd2  && Samp_mode_buf == 3'd2 ;
...
//    assign sha_hold = (cur_state == 2'd2) && is_rej_samp && (count_shake > 2'd0) && (count_shake < shake_num);
			...
			case (SHA3_mode)
				...
				4'd3: begin	// SHAKE256
					case (Samp_mode_buf)
						...
						3'd3: shake_num	<= 8'd15 ;								// Dilithum_gamma_samp
	...
	always @(*) begin
		if (cur_state == 2'd2) begin
			case (Samp_mode_buf)
				...
				3'd3: shake_eff	<= Dilithum_shake256_gamma_eff_wd;
				3'd4: shake_eff	<= 1'b0;
	...
	always @(*) begin
		case (Samp_mode_buf)
			...
			3'd1: begin
				sample_num1	<= {Dilithum_out_shake128_Uni[30:24],Dilithum_out_shake128_Uni[39:32],Dilithum_out_shake128_Uni[47:40]} ;
				sample_num2	<= {Dilithum_out_shake128_Uni[6:0],Dilithum_out_shake128_Uni[15:8],Dilithum_out_shake128_Uni[23:16]};
			end
			3'd2: begin
				sample_num1	<= Dilithum_out_shake128_Rej[3:0];
				sample_num2	<= Dilithum_out_shake128_Rej[7:4];
			end
			3'd3: begin
	            sample_num1	<= {Dilithum_out_shake256_gamma[49:48],Dilithum_out_shake256_gamma[63:56],Dilithum_out_shake256_gamma[71:64]} ;
				sample_num2	<= {Dilithum_out_shake256_gamma[35:32],Dilithum_out_shake256_gamma[47:40],Dilithum_out_shake256_gamma[55:50]};
	            sample_num3	<= {Dilithum_out_shake256_gamma[21:16],Dilithum_out_shake256_gamma[31:24],Dilithum_out_shake256_gamma[39:36]} ;
				sample_num4	<= {Dilithum_out_shake256_gamma[7:0],Dilithum_out_shake256_gamma[15:8],Dilithum_out_shake256_gamma[23:22]};
			end
			3'd4: begin
				sample_num1	<= 23'd0;
				sample_num2	<= 23'd0;
			...
			3'd5: begin
				sample_num1	<= {Kyber_out_shake128_Uni[11:8],Kyber_out_shake128_Uni[23:16]};
				sample_num2	<= {Kyber_out_shake128_Uni[7:0],Kyber_out_shake128_Uni[15:12]};
			end
			3'd6: begin
			    case (Samp_eta_buf)
					1'd0: begin
						sample_num1	<= Kyber_out_shake256_CBD[0] +  Kyber_out_shake256_CBD[1] ;
						sample_num2	<= Kyber_out_shake256_CBD[2] +  Kyber_out_shake256_CBD[3] ;
						sample_num3	<= Kyber_out_shake256_CBD[4] +  Kyber_out_shake256_CBD[5] ;
						sample_num4	<= Kyber_out_shake256_CBD[6] +  Kyber_out_shake256_CBD[7] ;
					end
					1'd1: begin
						sample_num1	<= Kyber_out_shake256_CBD[0] +  Kyber_out_shake256_CBD[1] +  Kyber_out_shake256_CBD[2];
						sample_num2	<= Kyber_out_shake256_CBD[3] +  Kyber_out_shake256_CBD[4] +  Kyber_out_shake256_CBD[5];
						sample_num3	<= Kyber_out_shake256_CBD[6] +  Kyber_out_shake256_CBD[7] +  Kyber_out_shake256_CBD[8];
						sample_num4	<= Kyber_out_shake256_CBD[9] +  Kyber_out_shake256_CBD[10] +  Kyber_out_shake256_CBD[11];
					end
					default: begin
						sample_num1	<= 23'd0;
						sample_num2	<= 23'd0;
...
//    //两路采样器
//    assign sample_num1 = sample_sel_buf ? Kyber_out_shake256_Rej[3:0] : {Kyber_out_shake128_Uni[15:12],Kyber_out_shake128_Uni[23:16]} ;
//    assign sample_num2 = sample_sel_buf ? Kyber_out_shake256_Rej[7:4] : {Kyber_out_shake128_Uni[3:0],Kyber_out_shake128_Uni[11:4]} ;
...
//    //拒绝采样判断
//    assign label_num1 = shake_eff_buf?(sample_sel_buf? (Samp_eta_buf? sample_num1_buf[3:0]<3'd7 : sample_num1_buf[3:0]<4'd15):( sample_num1_buf < 12'd3329)):1'b0;
//    assign label_num2 = shake_eff_buf?(sample_sel_buf? (Samp_eta_buf? sample_num2_buf[3:0]<3'd7 : sample_num2_buf[3:0]<4'd15):( sample_num2_buf < 12'd3329)):1'b0;
    ...
    always @(*) begin
		case (Samp_mode_buf)
			...
			3'd3: begin
				out_gamma_1 = 19'd131072 - sample_num1_buf2;
				out_gamma_2 = 19'd131072 - sample_num2_buf2;
				out_gamma_3 = 19'd131072 - sample_num3_buf2;
				out_gamma_4 = 19'd131072 - sample_num4_buf2;
			end
			3'd4: begin
				out_Rej_1 = 1'd0;
				out_Rej_2 = 1'd0;
	...
	assign out_gamma_final = out_gamma;
...
//    assign is_rej_samp =  sample_sel_buf;
...
//        Dilithum_shake256_gamma_eff_rd_reg0 <= Dilithum_shake256_gamma_eff_rd;
//		Dilithum_shake256_gamma_eff_rd_reg1 <= Dilithum_shake256_gamma_eff_rd_reg0;
//		Dilithum_shake256_gamma_eff_rd_reg2 <= Dilithum_shake256_gamma_eff_rd_reg1;   
		
        Dilithum_shake256_gamma_eff_wd_reg0 <= Dilithum_shake256_gamma_eff_wd;  	
		Dilithum_shake256_gamma_eff_wd_reg1 <= Dilithum_shake256_gamma_eff_wd_reg0;           
		Dilithum_shake256_gamma_eff_wd_reg2 <= Dilithum_shake256_gamma_eff_wd_reg1;   	
	    Dilithum_shake256_gamma_eff_wd_reg3 <= Dilithum_shake256_gamma_eff_wd_reg2;	
	    Dilithum_shake256_gamma_eff_wd_reg4 <= Dilithum_shake256_gamma_eff_wd_reg3;	
	    
//	    Dilithum_shake256_gamma_eff_reg0 <= Dilithum_shake256_gamma_eff_wd;  	
//		Dilithum_shake256_gamma_eff_reg1 <= Dilithum_shake256_gamma_eff_reg0;           
//		Dilithum_shake256_gamma_eff_reg2 <= Dilithum_shake256_gamma_eff_reg1;   	
//	    Dilithum_shake256_gamma_eff_reg3 <= Dilithum_shake256_gamma_eff_reg2;	
//	    Dilithum_shake256_gamma_eff_reg4 <= Dilithum_shake256_gamma_eff_reg3;	
...
//			3'd2: begin
//			    if (count_shake < shake_num && cur_state == 2'd2 && Samp_mode_buf == 3'd3)
//			      begin      
//                        if ( cycle_counter == 2'd1) begin
//                            count_shake <= count_shake + 1'b1; // 每两周期加一
//                             cycle_counter <= 2'd0; 
//                        end
//                        else begin
//                            count_shake <= count_shake; // 当前周期保持不变
//                             cycle_counter <=  cycle_counter + 1'b1; // 计数周期
...
//			end
		...
		//gamma输出
		if (cycle_counter == 2'd1) begin
			out_gamma		<= {out_gamma_3,out_gamma_4}; 
			cycle_counter	<= 2'd0; 
		end
		else begin
			cycle_counter	<= cycle_counter + 1'b1; 
			out_gamma		<= {out_gamma_1,out_gamma_2}; 
		end
       
        if (is_last)
            squeeze <= 1'b1;
        else if (end_flag | SHA3_Samp_start)
            squeeze <= 1'b0;
        else
            squeeze <= squeeze;
			...
			case (SHA3_mode)
				...
				4'd3: begin		// SHAKE256
					case (Samp_mode_buf)
						...
						3'd3: out_count	<= out_count - Dilithum_shake256_gamma_eff_wd_reg4;	// gamma采样
		...
		case (SHA3_mode)
			...
			4'd3: begin	// SHAKE256
				case (Samp_mode_buf)
					...
					3'd3: m_axis_tdata	<= {14'd0, out_gamma_final[35:18], 14'd0, out_gamma_final[17:0]};
        	...
        	case(SHA3_mode)
				...
				4'd3: begin		// SHAKE256
					case (Samp_mode)
						...
						3'd3: m_axis_tvalid	<= Dilithum_shake256_gamma_eff_wd_reg3;																								
