...
module SHA3_FIFO_read(
	...
	output	reg				Keccak_last,	// SHA3输入last信号
	...
	always @(posedge clk or negedge aresetn) begin
		if (!aresetn) begin
			...
			// 适配AHB_LITE
			// Keccak_last	<= Read_FIFO_tlast;
			if (read_counter == 32'd16 && rd_en) begin
				Keccak_last	<= 1'b1;
			end
			else begin
				Keccak_last	<= 1'b0;
			...
			if (module_start) begin	// start拉高，重置read_counter，rd_en为1时每次减8，直到置零
				if (add_empty) begin
