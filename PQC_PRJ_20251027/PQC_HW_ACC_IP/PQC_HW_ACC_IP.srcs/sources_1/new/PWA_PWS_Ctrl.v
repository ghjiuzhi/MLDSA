...
module PWA_PWS_Ctrl(
	...
	input	wire			PWA_PWS_start,
	input	wire			PWA_PWS_sel,		// '=0': add; '=1': sub
	...
	always @(posedge clk) begin
		...
		if (!PQC_mode) begin	// dilithum
			...
			data_in_1		<= input_data[31] ? (input_data[31:0] + 12'd3329) : input_data[11:0];
			data_in_2		<= input_data[63] ? (input_data[63:32] + 12'd3329) : input_data[43:32];
			add_result_1	<= (temp_result_1_buf > 23'd3329) ? temp_result_1_buf - 23'd3329 : temp_result_1_buf[22:0];
			add_result_2	<= (temp_result_2_buf > 23'd3329) ? temp_result_2_buf - 23'd3329 : temp_result_2_buf[22:0];			
		end
	...
	always @(*) begin
		case(cur_state)
			2'b00:
				if(PWA_PWS_start == 1'b1)
					next_state <= 2'b01;
				else
					next_state <= 2'b00;
			2'b01:
				if (read_first_done == 1'b1) begin
					if(PWA_PWS_length == 1)
						next_state <= 2'b11;
					else
						next_state <= 2'b10;
	...
	assign ram_sel_1 = write_working & (PWA_PWS_length[0]);
	assign ram_sel_2 = write_working & (!PWA_PWS_length[0]);
	...
	assign coef_douta_3 = (PWA_PWS_length[0]) ? coef_douta: temp_douta;
	assign coef_doutb_3 = (PWA_PWS_length[0]) ? coef_doutb: temp_doutb;
	...
	assign PWA_PWS_done = write_done;
	assign Write_FIFO_tkeep = 8'b11111111;
	assign Write_FIFO_tlast = write_done;

	read_mem_add mem_test2 (
		.clk			(clk),
		.module_start	(read_first_start),
		.Rm_tvalid		(Read_FIFO_tvalid),
		.data_in_1		(data_in_1),
		.data_in_2		(data_in_2),
		.rd_en			(Read_FIFO_tready_1),
		.coef_ena		(coef_ena_1),
		.coef_wea		(coef_wea_1),
		.coef_addra		(coef_addra_1),
		.coef_dina		(coef_dina_1),
		.coef_enb		(coef_enb_1),
		.coef_web		(coef_web_1),
		.coef_addrb		(coef_addrb_1),
		.coef_dinb		(coef_dinb_1),
		.module_done	(read_first_done)
	);
	
	compute_switch_add add_comput (
		.clk			(clk),
		.compute_start	(read_last_start),
		.compute_done	(read_last_done),
		.compu_working	(read_last_working),
		.PWA_PWS_length	(PWA_PWS_length),
		.coef_ena		(coef_ena_2),
		.coef_wea		(coef_wea_2),
		.coef_addra		(coef_addra_2),
		.coef_dina		(coef_dina_2),
		.coef_douta		(coef_douta),
		.coef_enb		(coef_enb_2),
		.coef_web		(coef_web_2),
		.coef_addrb		(coef_addrb_2),
		.coef_dinb		(coef_dinb_2),
		.coef_doutb		(coef_doutb),
		.temp_ena		(temp_ena_2),
		.temp_wea		(temp_wea_2),
		.temp_addra		(temp_addra_2),
		.temp_dina		(temp_dina),
		.temp_douta		(temp_douta),
		.temp_enb		(temp_enb_2),
		.temp_web		(temp_web_2),
		.temp_addrb		(temp_addrb_2),
		.temp_dinb		(temp_dinb),
		.temp_doutb		(temp_doutb),
		.Rm_tvalid		(Read_FIFO_tvalid),
		.data_in_1		(data_in_1),
		.data_in_2		(data_in_2),
		.Rm_tready		(Read_FIFO_tready_2),
		.add_num_1_1	(add_num_1_1_com),
		.add_num_1_2	(add_num_1_2_com),
		.add_num_2_1	(add_num_2_1_com),
		.add_num_2_2	(add_num_2_2_com),
		.add_result_1	(add_result_1),
		.add_result_2	(add_result_2)
	);
	
	compute_write_add add_write (
		.clk				(clk),
		...
		.coef_ena			(coef_ena_3),
		.coef_wea			(coef_wea_3),
		.coef_addra			(coef_addra_3),
		.coef_douta			(coef_douta_3),
		.coef_enb			(coef_enb_3),
		.coef_web			(coef_web_3),
		.coef_addrb			(coef_addrb_3),
		.coef_doutb			(coef_doutb_3),
		.Rm_tvalid			(Read_FIFO_tvalid),
		.data_in_1			(data_in_1),
		.data_in_2			(data_in_2),
		.Rm_tready			(Read_FIFO_tready_3),
		.Write_FIFO_tready	(Write_FIFO_tready),
		.Write_FIFO_tdata	(Write_FIFO_tdata),
		.Write_FIFO_tvalid	(Write_FIFO_tvalid),
		.add_num_1_1		(add_num_1_1_wri),
		.add_num_1_2		(add_num_1_2_wri),
		.add_num_2_1		(add_num_2_1_wri),
		.add_num_2_2		(add_num_2_2_wri),
		.add_result_1		(add_result_1),
		.add_result_2		(add_result_2)
	);
